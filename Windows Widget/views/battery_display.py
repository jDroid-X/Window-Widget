...
class BatteryCard(HardwareCard):
    def __init__(self, settings, click_action=None):
        super().__init__("battery", "PWR", click_action)
    ...
    def update_display(self, metrics, is_vertical, prog_cols):
        ...
        if bat.get("present", False):
            ...
            if bat.get("power_plugged"):
                status_str = "AC Connected / Charging"
            elif secsleft > 0:
                h_left = secsleft // 3600
                m_left = (secsleft % 3600) // 60
                status_str = f"On Battery ({h_left}h {m_left}m left)"
            else:
                status_str = "On Battery"
            self.set_metrics(f"{pct}%", pct, prog_cols, sub_str=status_str)
        else:
            self.hide()
